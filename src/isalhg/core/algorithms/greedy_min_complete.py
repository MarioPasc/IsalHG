...
``tests/unit/core/test_greedy_min_complete.py``). This variant instead
recurses over every tied candidate (``tie_branch=True``) and keeps the
lex-min completion, which removes edge ids from observable behaviour: the
resulting ``w*`` is invariant under vertex relabelling *and* edge
reordering, and is a complete isomorphism invariant (Theorem A of the
metric-space article; proof archived under the ISAL proofs directory).
...
same algorithm through ``canonical_string(algorithm="greedy_min_complete")``,
which routes to the C++ ``AlgorithmVariant::GreedyMinComplete`` single-FFI
fast path (multi-seed fan-out across the thread pool). Both produce
byte-identical output; keeping the reference reachable makes
``encode()`` vs ``canonical_string()`` a live differential assertion.
"""
...
class GreedyMinComplete(H2SAlgorithm):
    ...
    def encode(self, H: SparseHypergraph) -> TokenSequence:
        ...
        if not H.is_connected():
            raise DisconnectedHypergraphError(
                "GreedyMinComplete requires a connected hypergraph (decision B11)"
            )
        seeds = max_neighbor_degree_nodes(H)
        candidates: list[TokenSequence] = [
            _python_greedy_h2s(H, seed_node=s, k=self._k, inplace=True, tie_branch=True)
            for s in seeds
        ]
...
register_algorithm(
    "greedy_min_complete",
    lambda k, d: GreedyMinComplete(k=k, structural_depth=d),
)
