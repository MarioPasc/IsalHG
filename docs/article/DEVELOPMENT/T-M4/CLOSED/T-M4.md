...
- Arity gate (round-2 fix): `add_incidence` can grow hyperedge arity past k; _build now rejects candidates whose max arity exceeds `self._k`, preventing Σ_HG(k) violations and the K_MAX=10 C++ encoder crash seen in Picasso job 1547134_4. Regression test `TestArityGate` demonstrates the raw hazard (violations > 0 in 300 trials) then confirms the gate eliminates it (all 5 members of a k=3 family at n_edits=12 have max_arity <= 3). Closing checks round 2: pytest -m "not slow" --hypothesis-seed=0: 741 passed, 8 skipped; ruff 3; mypy 20.

---
**v3 rescope note (D-ART2, 2026-07-18; appended at the S1 reconciliation
merge).** The v3 re-spec of this task adds three deltas beyond what was
executed: (a) geometry-sweep parameterization of the planted families
(density, arity mix, size — `DATA.md` §1 / DQ5); (b) a
`metric_space/metrics/geometry.py` static-invariant module (concentration
stats + hubness skewness) plus `ν`/PSD reporting in `embedding`; (c) Pearson
added, MI retired. Ownership of the deltas: (a)+(b) are specced and completed
by **T-M5f**; (c) is verified there. The executed work and this closure stand
unchanged.
